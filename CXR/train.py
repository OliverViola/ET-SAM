...
def load_model(ckpt='/root/sam2/checkpoints/sam2.1_hiera_small.pt', cfg='configs/sam2.1/sam2.1_hiera_s.yaml'):
    ...
    for name, param in model.named_parameters():
        if name in old_param_names and 'mask_tokens' not in name and 'iou_token' not in name and 'point_embeddings' not in name:
...
def k_fold_train(k):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(np.arange(len(DATASET)))):
        ...
        scheduler = CosineAnnealingWarmRestarts(
            optimizer,
            T_0=20,           # 初始周期长度（例如50个epoch）
            T_mult=1,         # 周期长度倍增因子（每个周期后T=T*T_mult）
            eta_min=1e-6
        )
        train(fold, predictor, train_loader, val_loader, optimizer, scaler, scheduler, writer)
...
def train(fold, predictor:SAM2ImagePredictor, train_loader, val_loader, optimizer, scaler, scheduler, writer):
    # training iteration
    max_iou = 0
    max_dice = 0
    train_step = 0
